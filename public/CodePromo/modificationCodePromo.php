<?php
require_once "../../src/traitement/newCodePromo.php";
require_once "../../src/repository/CodePromoRepository.php";

if(isset($_POST['id'])){
    $id = $_POST["id"];
}
else if (isset($_GET['id'])){
    $id = $_GET["id"];
}
$cpr = new CodePromoRepository();
if(isset($code)){
    $cpr->modifierCodePromo($code);
}

$o=$cpr->getCodePromo($id);






?>
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <title>Cinémoi - Modification Code Promo</title>
    <meta name="viewport" content="width=device-width, initial-scale=1"/>
</head>
<body>
<form action="modificationCodePromo.php?message=Code Promo modifié !" method="post">
    <?php if(isset($_GET["message"])){?>
        <h5><?php echo $_GET["message"]?></h5>
    <?php }?>
    <div class="row">
        <div class="col-md-6">
            <div class="mb-3">
                <label>Pourcentage de reduction :</label>
                <input type="number" name="pourcentage_reduc" step="0.01" min="0" value="<?php echo $o->getPourcentageReduction()?>" required>
            </div>
            <div class="mb-3">
                <label>Code :</label>
                <input name="code" type="text" value="<?php echo $o->getCodePromo()?>" required>
            </div>
            <div class="mb-3">
                <label>Etat :</label>
                <?php if ($o->getEtat()==1){?>
                    <select name="etat">
                        <option value="1">Actif</option>
                        <option value="0">Inactif</option>
                    </select>
                <?php } else {?>
                    <select name="etat">
                        <option value="0">Inactif</option>
                        <option value="1">Actif</option>
                    </select>
                <?php } ?>
            </div>
            <input type="hidden" name="id" value="<?php echo $id ?>">
            <button type="submit">Valider</button>
            <button formaction="tabCodePromo.php">Retour</button>


</form>
</body>





</html>